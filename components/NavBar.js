    ...
    <nav className='bg-gray-900 text-white flex justify-between px-4 h-16 items-center'>
      ...
      <div >
        {session && <>
          ...
          <button id="dropdownDefaultButton" onClick={() => setShowdropdown(!showdropdown)} data-dropdown-toggle="dropdown" className="text-white mx-5 bg-blue-800 hover:bg-blue-900 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">
            Welcome {session.user.email}
            <svg className="w-4 h-4 ms-1.5 -me-0.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="m19 9-7 7-7-7" /></svg>
          </button> 

          
          <div id="dropdown" className={`z-10 ${showdropdown?"block":"hidden"} absolute top-15 right-80 border border-[1px] bg-slate-900 shadow-lg w-44`}>
